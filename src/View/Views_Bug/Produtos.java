...
public class Produtos{
    ...
    public Produtos(){ //
        ...
        produtos.add(new Produto("tomate", 2.5f, 7));
    }
    ...
    public void comprarProduto(ComprarProduto comprarProduto){
        for(int i = 0; i < produtos.size(); i++){
            ...
            if(produto.trazerNome().toLowerCase().equals(comprarProduto.mostrarNome()) && produto.trazerQuantidade() > 0){
                if(produto.trazerQuantidade() >= comprarProduto.mostrarQuantidade()){
                    ...
                    Float custoProduto = produto.trazerValor() * comprarProduto.mostrarQuantidade();
                    ...
                    //Realizando atualização nos produtos do mercado
                        produto.alterarQuantidade(produto.trazerQuantidade() - comprarProduto.mostrarQuantidade());
                        if (produto.trazerQuantidade() == 0){
    ...
    public void removerProduto(RemoverProduto removerProduto){
        for(int i = 0; i < produtos.size(); i++){
            ...
            if(produto.trazerNome().toLowerCase().equals(removerProduto.mostrarNomeR())){
    ...
    public void pesquisarProduto(PesquisarProduto pesquisarProduto){
        for (Produto produto: produtos){ //Chamada Enhanced for-loop para percorrer toda a lista.
            if(produto.trazerNome().toLowerCase().equals(pesquisarProduto.mostrarNomeP().toLowerCase())){
